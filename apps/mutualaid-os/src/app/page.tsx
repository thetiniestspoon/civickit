...
import { AppShell, SafeToUseBanner, SearchBar, ResourceCard } from "@civickit/ui";
import type { ResourceType } from "@civickit/types";
import { demoResources } from "@/data/demo-resources";

const navItems = [
  { label: "Find Help", href: "/", active: true },
          ...
          {filteredResources.length > 0 ? (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {filteredResources.map((resource) => (
                <a key={resource.id} href={`/resources/${resource.id}`} className="block hover:no-underline">
                  <ResourceCard
                    name={resource.name}
                    type={resource.type}
